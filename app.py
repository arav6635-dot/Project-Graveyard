...
if not DATABASE_URL:
    app.logger.warning("DATABASE_URL is not set. Neon/Postgres connection will fail.")
...
def db() -> psycopg.Connection:
    if not DATABASE_URL:
        raise RuntimeError("DATABASE_URL is required for Neon/Postgres mode.")
...
def ensure_default_causes(conn: psycopg.Connection) -> None:
    for cause in DEFAULT_CAUSES:
        conn.execute(
            "INSERT INTO failure_causes(name) VALUES(%s) ON CONFLICT(name) DO NOTHING",
            (cause,),
        )
...
def cleanup_cause_data(conn: psycopg.Connection) -> None:
    rows = conn.execute("SELECT id, name FROM failure_causes ORDER BY id ASC").fetchall()
    for row in rows:
        old_id = row["id"]
        normalized = normalize_cause_name(row["name"])
        if not normalized:
            conn.execute("DELETE FROM project_cause_votes WHERE cause_id = %s", (old_id,))
            conn.execute("DELETE FROM failure_causes WHERE id = %s", (old_id,))
            continue
        ...
        new_id = conn.execute(
            "SELECT id FROM failure_causes WHERE name = %s",
            (normalized,),
        ).fetchone()["id"]
        ...
        votes = conn.execute(
            "SELECT project_id, votes FROM project_cause_votes WHERE cause_id = %s",
            (old_id,),
        ).fetchall()
        for vote in votes:
            conn.execute(
...
def seed_projects_from_sql(conn: psycopg.Connection) -> None:
    if not SEED_SQL_PATH.exists():
        return
    run_sql_script(conn, SEED_SQL_PATH.read_text(encoding="utf-8"))
...
def project_with_meta(conn: psycopg.Connection, project_id: int) -> dict[str, Any] | None:
    ...
    tags = conn.execute(
        "SELECT tag FROM project_tags WHERE project_id = %s ORDER BY tag ASC",
        (project_id,),
    ).fetchall()
...
def build_portfolio_snapshot(conn: psycopg.Connection) -> list[dict[str, Any]]:
...
def list_projects():
    ...
    conn = db()
    rows = conn.execute(
        f"SELECT id FROM projects {where} ORDER BY created_at DESC, id DESC", tuple(params)
    ).fetchall()
...
def create_project():
    ...
    for tag in payload.get("tags", []):
        ...
        if clean:
            conn.execute(
                "INSERT INTO project_tags(project_id, tag, created_at) VALUES(%s, %s, %s) ON CONFLICT(project_id, tag) DO NOTHING",
    ...
    for cause_name in payload.get("initial_causes", []):
        ...
        cause_id = conn.execute(
            "SELECT id FROM failure_causes WHERE name = %s", (clean,)
        ).fetchone()["id"]
        conn.execute(
            """
            INSERT INTO project_cause_votes(project_id, cause_id, votes, source)
...
def vote_cause(project_id: int):
    ...
    cause_id = conn.execute(
        "SELECT id FROM failure_causes WHERE name = %s", (cause_name,)
    ).fetchone()["id"]
...
def analyze(project_id: int):
    ...
    conn.execute(
        "INSERT INTO analysis_reports(project_id, model, analysis_json, created_at) VALUES(%s, %s, %s, %s)",
        (
            project_id,
...
def analytics_overview():
    ...
    cached = conn.execute(
        "SELECT payload_json FROM ai_analytics_cache WHERE fingerprint = %s",
        (fingerprint,),
    ).fetchone()
